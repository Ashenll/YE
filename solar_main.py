...
class SolarSystem:
    ...
    def execution(self):
        ...
        SpaceModel.recalculate_space_objects_positions(self.space_objects, self.time_step.get())
        for body in self.space_objects:
            SpaceVis.update_object_position(self.space, body)
    ...
    def open_file_dialog(self):
        ...
        self.space_objects = SpaceInput.read_space_objects_data_from_file(in_filename)
        max_distance = max([max(abs(obj.x), abs(obj.y)) for obj in self.space_objects])
        SpaceVis.calculate_scale_factor(max_distance*2)

        for obj in self.space_objects:
            if obj.type == 'star':
                SpaceVis.create_star_image(self.space, obj)
            elif obj.type == 'planet':
                SpaceVis.create_planet_image(self.space, obj)
            else:
    ...
    def save_file_dialog(self):
        ...
        SpaceInput.write_space_objects_data_to_file(out_filename, self.space_objects)
    def main(self):
        ...
        self.space = tkinter.Canvas(self.root, width=SpaceVis.window_width, height=SpaceVis.window_height, bg="black")
